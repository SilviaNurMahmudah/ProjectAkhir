      ...
      <a class="navbar-brand js-scroll-trigger" href="#page-top"><img src="assets/img/navbar-logo.svg" alt="" /></a>
        ...
        <div class="collapse navbar-collapse" id="navbarResponsive">
          <ul class="navbar-nav text-uppercase ml-auto">
            ...
            <li class="nav-item"><a class="nav-link js-scroll-trigger" href="#">Edit</a></li>
          </ul>
        </div>
    </div>
  </nav>
  <main id="main">
    ...
    <section class="page-section" id="contact">
      <div class="container">
        ...
        <form class="php-email-form mt-4" action="/Destinasi/update/{{$Destinasi->id}}" method="post" enctype="multipart/form-data">
        {{csrf_field()}}
        <input type="hidden" name="id" value="{{$Destinasi->id}}"></br>
        <div class="form-group">
            <label for="name"><font color="white">Title</font></label>
            <input type="text" class="form-control" required="required" name="title" value="{{$Destinasi->title}}">
        </div>
        <div class="form-group">
            <label for="title"><font color="white">Content</font></label>
            <input type="text" class="form-control" required="required" name="content" value="{{$Destinasi->content}}">
        </div>
        <div class="form-group">
            <label for="content"><font color="white">Jenis Wisata</font></label>
            <input type="text" class="form-control" required="required" name="jenis_wisata" value="{{$Destinasi->jenis_wisata}}">
        </div>
        <div class="form-group">
            <label for="image"><font color="white">Image</font></label>
            <input type="file" class="form-control" required="required" name="image" value="{{$Destinasi->image}}"><br>
        </div>
        <div>
            <img width="150px" src="{{asset('storage/'.$Destinasi->image)}}">
        </div>
        <div class="text-center">
            <div id="success"></div>
